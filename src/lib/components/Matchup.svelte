...
<script lang="ts">
	...
	function flipCoin() {
		...
		for (let i = 0; i < COIN_TOGGLES; i++) {
			timers.push(window.setTimeout(() => (coinSide = i % 2 === 0 ? "a" : "b"), i * COIN_STEP_MS));
		}
		timers.push(
			window.setTimeout(
				() => {
					coinSide = null;
					decided = final;
					timers.push(window.setTimeout(() => ondecide(final), DECIDE_DELAY_MS));
				},
...
{#if bout.a && bout.b}
	<div class="matchup">
		...
		<div class="matchup__coin">
			<button type="button" class="btn-ghost btn-small" onclick={flipCoin} disabled={locked}>
