<script lang="ts">
  ...
  // Analytics boots on mount so it never runs during prerendering. PostHog
  // takes pageviews from here on itself (history_change), so routes only send
  // the explicit game events.
...
<div class="app">
  ...
  {#if game.showBracket && game.rounds}
    <BracketView
      rounds={game.rounds}
      onclose={() => (game.showBracket = false)}
    />
  {/if}
</div>
